...
use crate::{
    ...
    SelectMiniKind, SelectionExtraInfo, Theme, ViewString, select::complex::NetworkKind, text,
    window_inner_height,
};
...
impl Model {
            ...
            <div id={ctx.props().id.clone()} class="complex-select-pop" style={style_pop}>
                ...
                </div>
                {
                    if cfg!(feature = "pumpkin") {
                        ...
                        let input_group_style = format!("min-height: {input_group_min_height}px;");
                        html! {
                        ...
                        html! {
                            {
                                match ctx.props().kind {
                                    SelectComplexKind::NetworkIp => {
                                        let onclick_all = ctx
                                            .link()
                                            .callback(|_| Message::ClickAllBelow(NetworkKind::Registered));
                                        html! {
                                            <>
                                                <div class="complex-select-pop-list-select-all-group">
                                                    ...
                                                    { self.view_direction(ctx, NetworkKind::Registered) }
                                                </div>
    ...
    fn view_direction(&self, ctx: &Context<Self>, origin: NetworkKind) -> Html {
        let (parent_message, id, active) = match origin {
            ...
            NetworkKind::Custom => {
                ...
                (
                    Message::SetDirectionItem(NetworkKind::Custom),
    ...
    fn view_list_item(&self, ctx: &Context<Self>, item: &NetworkItem) -> Html {
        ...
        let onclick_item = |key: String| {
            ctx.link()
                .callback(move |_| Message::ClickItem(key.clone(), NetworkKind::Registered))
        };
            ...
            if let Some(selected) = self.direction_items.get(id) {
                html! {
                    <SelectMini::<SelectionExtraInfo, Self>
                        txt={ctx.props().txt.clone()}
                        language={ctx.props().language}
                        parent_message={Message::SetDirectionItem(NetworkKind::Registered)}
                    ...
                    </div>
                    {
                        if custom_is_empty {
                            ...
                            let onclick_all_custom = ctx
                                .link()
                                .callback(|_| Message::ClickAllBelow(NetworkKind::Custom));
                            let theme = ctx.props().theme;
                            html! {
                                <>
                                    <div class="complex-select-pop-list-select-all-group">
                                        ...
                                        { self.view_direction(ctx, NetworkKind::Custom) }
                                    </div>
