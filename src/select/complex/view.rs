...
impl Model {
    ...
    pub(super) fn view_pop(&self, ctx: &Context<Self>) -> Html {
        ...
        let style_all = match self.check_status(ctx, false) {
            CheckStatus::Checked => {
                if cfg!(feature = "pumpkin-dark") {
                    "background-image: url('/frontary/clumit-radio-opener-checked.png');"
                } else {
                    "background-image: url('/frontary/radio-opener-checked.png');"
                }
            }
            _ => {
                if cfg!(feature = "pumpkin-dark") {
                    "background-image: url('/frontary/clumit-radio-opener-unchecked.png');"
                } else {
                    "background-image: url('/frontary/radio-opener-unchecked.png');"
                }
            }
        };
        ...
        let style_input = if self.view_list {
            if cfg!(feature = "pumpkin-dark") {
                "background-image: url('/frontary/clumit-collapse-contents.png');"
            } else {
                "background-image: url('/frontary/collapse-contents.png');"
                                ...
                                html! {
                                    match ctx.props().kind {
                                        SelectComplexKind::NetworkIp => {
                                            ...
                                            let top_bg_color = if cfg!(feature = "pumpkin-dark") {
                                                "rgba(97, 105, 116, 0.24)"
                                            } else {
                                                "#F6F6F6"
                                            };
