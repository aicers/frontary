...
impl Model {
    ...
    fn view_network_ip_item_direction(
        ...
        if checked {
            ...
            let top_width = if cfg!(feature = "appearance") { 90 } else { 70 };
    ...
    fn view_input(&self, ctx: &Context<Self>) -> Html {
        ...
        let style_width_input = if cfg!(feature = "appearance") {
                        ...
                        for keys.iter().map(|&key| {
                            if let Some(value) = custom.get(key) {
                                html! {
                                    match ctx.props().kind {
                                        SelectComplexKind::NetworkIp => {
                                            let style_ip = if cfg!(feature = "appearance") {
                                            ...
                                            let (top_bg_color, top_width, list_top) = if cfg!(feature = "appearance") {
