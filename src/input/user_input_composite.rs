    ...
    <T as Component>::Message: Clone + PartialEq,
{
            ...
            InputConfig::Group(config) => {
                html! {
                    <div class={class_child}>
                        ...
                        { self.view_group(ctx, &config.ess, config.all_in_one_row, &config.widths, &config.items, child_data, Some(base_index), layer_index, config.group_kind.as_deref(),) }
                    </div>
                }
            }
    ...
    pub(super) fn view_group(
        ...
        let (bottom_line_class, input_group_table_class) = match group_kind {
            Some("compact") => (
                        ...
                        <table class={input_group_table_class}>
                            ...
                            }
                            {
                                for input_data.iter().enumerate().map(move |(row_index, row)| {
                                    ...
                                    let is_compact_group = group_kind == Some("compact");
                                    let top_line_class = match (is_compact_group, row_index == 0, row_has_error) {
