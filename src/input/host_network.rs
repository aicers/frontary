    ...
    <T as Component>::Message: Clone + PartialEq,
{
    ...
    fn view_item(ctx: &Context<Self>, index: DeleteIndex, item: &str, theme: Theme) -> Html {
        let onclick_delete =
            |index: DeleteIndex| ctx.link().callback(move |_| Message::Delete(index));
        let img = if cfg!(feature = "pumpkin") {
            theme.themed_path("delete-x.svg")
        } else {
            theme.themed_path("host-network-close.png")
        };
        html! {
            <div class="host-network-group-input-item">
                { item }
                <img src={img} class="host-network-group-close" onclick={onclick_delete(index)} />
            </div>
        }
    }
