        ...
        html! {
            for (0..widths.len()).map(|i| {
                ...
                if let Some(title) = ctx.props().display_info.titles.get(index) {
                    ...
                    let style = if cfg!(feature = "appearance") {
                                ...
                                match ctx.props().kind {
                                    Kind::LayeredFirst => {
                                        if ctx.props().data_type == Some(DataType::Customer) {
                                            ...
                                            let (prefix, extension) = if cfg!(feature = "appearance") {
                                                ("pumpkin/", "svg")
                                            } else {
                                                ("", "png")
                                            };
                                            ...
                                            let list_top = if cfg!(feature = "appearance") {
                                                34
                                            } else {
                                                28
                                            };
                        ...
                        <tr>
                            <td class="list-whole-delete-checked-trash">
                                <div class="list-whole-delete-checked-trash" onclick={onclick_delete}>
                                    <img src={if cfg!(feature = "appearance") { "/frontary/pumpkin/delete-trash.svg" } else { "/frontary/delete-trash-white.png" }} class="list-whole-delete-trash-white" />
                                </div>
                            </td>
                            <td class="list-whole-delete-checked-close">
                                <div class="list-whole-delete-checked-close" onclick={onclick_cancel}>
                                    <img src={if cfg! (feature = "appearance") {"/frontary/pumpkin/close-white.svg"} else {"/frontary/close-white.png"} } class="list-whole-close-white" />
                                </div>
                            </td>
                        </tr>
                    </table>
