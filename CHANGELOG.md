...
- Removed `br_separator` for `Column::{Checkbox | Radio}`. - Introduced
  `{CheckboxColumn | RadioColumn}::modal`, which displays a modal window
  corresponding to each of the `{CheckboxColumn | RadioColumn}::display`
  elements.
...
[Unreleased]: https://github.com/aicers/frontary/compare/0.10.1...main
